...
public interface EditedMessageDao {
    ...
    @Query("SELECT EXISTS(SELECT * FROM editedmessage WHERE userId = :userId AND dialogId = :dialogId AND messageId = :messageId AND entityCreateDate = :entityCreateDate)")
    boolean existsRevision(long userId, long dialogId, long messageId, int entityCreateDate);
